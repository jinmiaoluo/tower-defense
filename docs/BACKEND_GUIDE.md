...
- 定义数据模型 (models.py)
- 编写计算器测试 (test_calculators.py)
- 实现计算器 (calculators.py)
- 编写验证器测试 (test_validators.py)
- 实现验证器 (validators.py)
- 编写 API 测试 (test_views.py)
- 实现 API 视图 (views.py)
...
**设计说明**：随机算法增加游戏变化性。服务端在生成时将完整的怪物配置（含 ID 和属性）下发给客户端，验证时通过怪物 ID 匹配。
...
- **服务端权威**：所有游戏配置和怪物属性由服务端定义，客户端不内置任何数值
- **怪物 ID 生成**：每波次怪物必须由服务端生成唯一 UUID，用于后续验证
- **验证顺序**：波次连续性 → Level 1 → Level 2 → 更新状态 → Level 4
- **事务处理**：波次提交应在数据库事务中完成，验证失败时回滚
- **状态一致性**：GameSession 的状态必须与所有 WaveRecord 的累计结果一致
- **会话清理**：定时任务每小时清理超过 24 小时的过期会话
