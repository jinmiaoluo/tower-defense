...
- **手动暂停**: 点击暂停按钮或按空格键，切换游戏暂停状态
- **自动暂停/恢复**: 页面切换到后台时自动暂停，返回时自动恢复

**自动暂停/恢复实现**:

- 使用 Page Visibility API 监听 `visibilitychange` 事件
- 补充使用 `pagehide`/`pageshow` 事件增强移动设备兼容性
- 页面隐藏时（`document.hidden === true`）自动暂停游戏
- 页面恢复可见时自动恢复游戏（仅限自动暂停的情况）
- 用户手动暂停后切换标签页，返回时不会自动恢复
- 使用 `wasAutoPaused` 标志区分自动暂停和手动暂停
- `pageshow` 事件仅在 `persisted` 为 `true`（从 bfcache 恢复）时触发自动恢复

**暂停期间的行为**:

- 帧号不增长，确保服务端验证时排除暂停时间
- 允许建筑操作（放置、升级、出售）
- 怪物和子弹停止移动
- 不处理波次完成和间隔计数
