...
怪物使用动态寻路算法（Dijkstra/BFS）从入口移动到出口。
...
> **来源**: 旧实现 `td-obj-monster.js:124-136`（寻路）、`td-walk.js`（寻路算法）、`td-obj-grid.js:47-77`（建筑阻塞检查）
...
```
实际速度 = speed × GLOBAL_SPEED
示例: 普通怪 speed=3 → 实际速度 = 3 × 0.1 = 0.3 像素/帧
```

**子弹飞行速度**:

```
实际速度 = bullet_speed × 20 × GLOBAL_SPEED
示例: cannon bullet_speed=6 → 实际速度 = 6 × 20 × 0.1 = 12 像素/帧
```
...
```
攻击间隔(帧) = floor(max(2 / (speed × GLOBAL_SPEED), 1))
示例:
- cannon (speed=2): floor(2 / 0.2) = 10 帧
- LMG/HMG (speed=3): floor(2 / 0.3) = 6 帧
- laser_gun (speed=20): floor(2 / 2) = 1 帧
```

> **设计说明**: 攻击间隔与建筑的 `speed` 属性成反比。`speed` 越高，攻击越频繁。公式保证最小间隔为 1 帧。来源: 旧实现 `td-obj-building.js:168`。
